package com.clarolab.connectors.impl.jenkins.report.jest;
...
@Log
public class JenkinsJestReport extends JenkinsBaseReport implements JenkinsReportType {

    @Builder
    private JenkinsJestReport(JenkinsBuild jenkinsBuild, ApplicationContextService context, String applicationTestingEnvironmentVersion, String cvsLogs) {
    ...
    public Report getReport(List<JenkinsArtifact> jenkinsArtifactList) throws ReportServiceException {
        ...
        return ReportUtils.builder().applicationTestingEnvironmentVersion(applicationTestingEnvironmentVersion).build().getReport(allReports, ReportType.JEST);
    }
    ...
    public Report createReport(String content) throws ReportServiceException {
        log.info(String.format("Found JEST report for %s", jenkinsBuild.getFullDisplayName()));
        return JestReport.builder().context(context).applicationTestingEnvironmentVersion(applicationTestingEnvironmentVersion).build().createReport(content, "This is a report for " + jenkinsBuild.getFullDisplayName());
    }
    ...
    private Report getReport(JenkinsArtifact artifact) throws ReportServiceException {
        try {
            ...
            throw ReportServiceException.builder().message(parseDataError("Error getting report", jenkinsBuild.getFullDisplayName())).reason("Reason: " + StatusType.getStatus(StatusType.UNKNOWN)).type(ReportType.JEST).exception(e).build();
        }
    }
