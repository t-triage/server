package com.clarolab.connectors.impl.jenkins.report.cypress;
...
@Log
public class JenkinsCypressReport extends JenkinsBaseReport implements JenkinsReportType {

    @Builder
    private JenkinsCypressReport(JenkinsBuild jenkinsBuild, ApplicationContextService context, String applicationTestingEnvironmentVersion, String cvsLogs) {
    ...
    public Report getReport(List<JenkinsArtifact> jenkinsArtifactList) throws ReportServiceException {
        ...
        return ReportUtils.builder().applicationTestingEnvironmentVersion(applicationTestingEnvironmentVersion).build().getReport(allReports, ReportType.CYPRESS);
    }
    ...
    public Report createReport(String content) throws ReportServiceException {
        log.info(String.format("Found CYPRESS report for %s", jenkinsBuild.getFullDisplayName()));
        return CypressReport.builder().context(context).applicationTestingEnvironmentVersion(applicationTestingEnvironmentVersion).build().createReport(content, "This is a report for " + jenkinsBuild.getFullDisplayName());
    }
    ...
    private Report getReport(JenkinsArtifact artifact) throws ReportServiceException {
        try {
            ...
            throw ReportServiceException.builder().message(parseDataError("Error getting report", jenkinsBuild.getFullDisplayName())).reason("Reason: " + StatusType.getStatus(StatusType.UNKNOWN)).type(ReportType.CYPRESS).exception(e).build();
        }
    }
